...
impl AgentInputFooter {
    ...
    pub fn new(
        ...
        let install_plugin_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Enable notifications", InstallPluginButtonTheme)
                ...
                .with_tooltip(
                    "Install the Smash plugin to enable rich agent notifications within Smash",
                )
                .with_size(cli_button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
        ...
        let plugin_instructions_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Notifications setup instructions", InstallPluginButtonTheme)
                .with_icon(Icon::Info)
                .with_tooltip("View instructions to install the Smash plugin")
                .with_size(cli_button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
                .with_adjoined_side(AdjoinedSide::Right)
                .on_click(|ctx| {
        ...
        let update_plugin_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Update Smash plugin", InstallPluginButtonTheme)
                .with_icon(Icon::Download)
                .with_tooltip("A new version of the Smash plugin is available")
                .with_size(cli_button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
                .with_adjoined_side(AdjoinedSide::Right)
                .on_click(|ctx| {
        ...
        let update_instructions_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Plugin update instructions", InstallPluginButtonTheme)
                .with_icon(Icon::Info)
                .with_tooltip("View instructions to update the Smash plugin")
                .with_size(cli_button_size)
                .with_tooltip_alignment(TooltipAlignment::Left)
                .with_adjoined_side(AdjoinedSide::Right)
                .on_click(|ctx| {
        ...
        Fut: std::future::Future<Output = Result<(), PluginInstallError>> + Send + 'static,
    {
        ...
        let (shell_path, shell_type) = match shell_data {
            ...
            // needing the Smash plugin to integrate with it.
    ...
    fn handle_install_plugin(&mut self, ctx: &mut ViewContext<Self>) -> bool {
        let success_msg = self
            ...
            .unwrap_or("Smash plugin installed. Please restart the session to activate.");
        self.handle_plugin_operation(
            "Installing Smash plugin...",
            "Failed to install Smash plugin",
            success_msg,
    ...
    fn handle_update_plugin(&mut self, ctx: &mut ViewContext<Self>) -> bool {
        let success_msg = self
            ...
            .unwrap_or("Smash plugin updated. Please restart the session to activate.");
        self.handle_plugin_operation(
            "Updating Smash plugin...",
            "Failed to update Smash plugin",
            success_msg,
...
/// Green-accented theme for the "Install Smash plugin" chip.
...
async fn write_install_log(agent: CLIAgent, err: &PluginInstallError) -> Option<PathBuf> {
    let log_path = env::temp_dir().join("smash-plugin-install.log");
    let now = chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC");
    let contents = format!(
        "Smash plugin installation — {agent:?}\n\
         {now}\n\
         \n\
         {log}",
        log = err.log,
