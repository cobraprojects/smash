...
pub(crate) const DEFAULT_SMASH_OLLAMA_URL: &str = "http://127.0.0.1:11434";
pub(crate) const DEFAULT_SMASH_LM_STUDIO_URL: &str = "http://127.0.0.1:1234";

static SMASH_OLLAMA_URL: LazyLock<RwLock<String>> =
    LazyLock::new(|| RwLock::new(DEFAULT_SMASH_OLLAMA_URL.to_owned()));
static SMASH_LM_STUDIO_URL: LazyLock<RwLock<String>> =
    LazyLock::new(|| RwLock::new(DEFAULT_SMASH_LM_STUDIO_URL.to_owned()));

pub(crate) fn normalize_smash_provider_url(value: &str) -> anyhow::Result<String> {
    let value = value.trim().trim_end_matches('/');
    let (_, authority_and_path) = value
        .split_once("://")
        .ok_or_else(|| anyhow!("provider URL must include http:// or https://"))?;
    if authority_and_path.is_empty() || authority_and_path.starts_with('/') {
        return Err(anyhow!("provider URL must include a server host"));
    }
    let url = url::Url::parse(value).context("provider URL is invalid")?;
    if !matches!(url.scheme(), "http" | "https") || url.host_str().is_none() {
        return Err(anyhow!("provider URL must be an HTTP or HTTPS server URL"));
    }
    Ok(value.to_owned())
}

pub(crate) fn set_smash_provider_urls(ollama: String, lm_studio: String) {
    *SMASH_OLLAMA_URL.write() = ollama;
    *SMASH_LM_STUDIO_URL.write() = lm_studio;
}

pub(crate) fn smash_ollama_url() -> String {
    SMASH_OLLAMA_URL.read().clone()
}

pub(crate) fn smash_lm_studio_url() -> String {
    SMASH_LM_STUDIO_URL.read().clone()
}
...
fn smash_model_info(id: &str, display_name: &str, provider: LLMProvider) -> LLMInfo {
    LLMInfo {
...
impl LLMPreferences {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        let models_by_feature = if ChannelState::channel() == Channel::Oss {
            smash_models_by_feature()
        } else {
    ...
    pub(crate) fn refresh_smash_local_models(&self, ctx: &mut ModelContext<Self>) {
        ...
        let lm_studio_base_url =
            normalize_smash_provider_url(AISettings::as_ref(ctx).smash_lm_studio_url.value())
                .unwrap_or_else(|_| DEFAULT_SMASH_LM_STUDIO_URL.to_owned());
        ...
        ctx.spawn(
            ...
            |me, (ollama, lm_studio), ctx| {
                ...
                choices.extend(lm_studio.into_iter().map(|model| {
                    smash_model_info(
                        &format!("lmstudio:{model}"),
                        &format!("{model} (LM Studio)"),
                        LLMProvider::Unknown,
                    )
                }));
                let available = AvailableLLMs {
                ...
                me.models_by_feature.agent_mode = available.clone();
                me.models_by_feature.coding = available.clone();
