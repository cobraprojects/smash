...
#[test]
fn smash_provider_url_normalizes_trailing_slashes() {
    assert_eq!(
        normalize_smash_provider_url(" http://127.0.0.1:11434/// ").unwrap(),
        "http://127.0.0.1:11434"
    );
    assert_eq!(
        normalize_smash_provider_url("https://models.example.com/api/").unwrap(),
        "https://models.example.com/api"
    );
}

#[test]
fn smash_provider_url_rejects_unsupported_or_hostless_urls() {
    assert!(normalize_smash_provider_url("file:///tmp/models").is_err());
    assert!(normalize_smash_provider_url("localhost:11434").is_err());
    assert!(normalize_smash_provider_url("http:///api").is_err());
}
