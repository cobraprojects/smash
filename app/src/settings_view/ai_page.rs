...
use crate::ai::llms::{
    DEFAULT_SMASH_LM_STUDIO_URL, DEFAULT_SMASH_OLLAMA_URL, LLMContextWindow, LLMId, LLMPreferences,
    LLMPreferencesEvent, LLMProvider, is_using_api_key_for_provider, normalize_smash_provider_url,
    set_smash_provider_urls,
};
...
impl AISettingsPageView {
    ...
    #[cfg(not(target_family = "wasm"))]
    fn save_smash_provider_url(
        &mut self,
        value: &str,
        is_ollama: bool,
        ctx: &mut ViewContext<Self>,
    ) {
        let normalized = match normalize_smash_provider_url(value) {
            Ok(url) => url,
            Err(error) => {
                let window_id = ctx.window_id();
                crate::ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                    stack.add_ephemeral_toast(
                        crate::view_components::DismissibleToast::error(error.to_string()),
                        window_id,
                        ctx,
                    );
                });
        ...
        let result = AISettings::handle(ctx).update(ctx, |settings, ctx| {
            if is_ollama {
                settings.smash_ollama_url.set_value(normalized.clone(), ctx)
            } else {
                settings
                    .smash_lm_studio_url
                    .set_value(normalized.clone(), ctx)
            }
        });
            ...
            crate::ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                stack.add_ephemeral_toast(
                    crate::view_components::DismissibleToast::error(format!(
                        "Could not save provider URL: {error}"
                    )),
                    window_id,
                    ctx,
                );
        ...
        let ollama = AISettings::as_ref(ctx).smash_ollama_url.value().clone();
        let lm_studio = AISettings::as_ref(ctx).smash_lm_studio_url.value().clone();
        set_smash_provider_urls(ollama, lm_studio);
        LLMPreferences::handle(ctx).update(ctx, |preferences, ctx| {
            preferences.refresh_smash_local_models(ctx);
        });

        // Recreate the provider inputs so the normalized saved URL remains visible after submit.
    ...
    fn knowledge_widgets() -> Vec<Box<dyn SettingsWidget<View = Self>>> {
        ...
        widgets
            .push(Box::new(ManageRulesWidget::default()) as Box<dyn SettingsWidget<View = Self>>);
        ...
        if ChannelState::channel() == Channel::Oss {
            let title = match subpage {
                None | Some(AISubpage::WarpAgent) => {
                    #[cfg(not(target_family = "wasm"))]
                    widgets.push(Box::new(SmashProvidersWidget::new(ctx)));
                ...
                Some(AISubpage::ThirdPartyCLIAgents) => {
                    ...
                    Some("Third party CLI agents")
                }
            };
            return PageType::new_uncategorized(widgets, title);
        }
...
pub enum AISettingsPageAction {
    ...
    ConnectSmashChatGPT,
    #[cfg(not(target_family = "wasm"))]
    DisconnectSmashChatGPT,
    #[cfg(not(target_family = "wasm"))]
    SetSmashOllamaUrl(String),
    #[cfg(not(target_family = "wasm"))]
    SetSmashLmStudioUrl(String),
...
impl TypedActionView for AISettingsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            #[cfg(not(target_family = "wasm"))]
            AISettingsPageAction::ConnectSmashChatGPT => {
                let attempt = match smash_chatgpt::OAuthAttempt::start() {
                    Ok(attempt) => attempt,
                    Err(error) => {
                        let window_id = ctx.window_id();
                        crate::ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                            stack.add_ephemeral_toast(
                                crate::view_components::DismissibleToast::error(format!(
                                    "Could not start ChatGPT login: {error}"
                                )),
                                window_id,
                                ctx,
                            );
                    ...
                    crate::ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                        let toast = match result {
                            Ok(()) => crate::view_components::DismissibleToast::success(
                                "ChatGPT connected to Smash".to_owned(),
                            ),
                            Err(error) => crate::view_components::DismissibleToast::error(format!(
                                "ChatGPT login failed: {error}"
                            )),
                        };
            ...
            #[cfg(not(target_family = "wasm"))]
            AISettingsPageAction::SetSmashOllamaUrl(value) => {
                self.save_smash_provider_url(value, true, ctx);
            }
            #[cfg(not(target_family = "wasm"))]
            AISettingsPageAction::SetSmashLmStudioUrl(value) => {
                self.save_smash_provider_url(value, false, ctx);
            }
...
#[cfg(not(target_family = "wasm"))]
struct SmashProvidersWidget {
    connect_chatgpt_button: ViewHandle<ActionButton>,
    disconnect_chatgpt_button: ViewHandle<ActionButton>,
    ollama_url_input: ViewHandle<SubmittableTextInput>,
    lm_studio_url_input: ViewHandle<SubmittableTextInput>,
}

#[cfg(not(target_family = "wasm"))]
impl SmashProvidersWidget {
    fn new(ctx: &mut ViewContext<AISettingsPageView>) -> Self {
        let connect_chatgpt_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Connect", SecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(AISettingsPageAction::ConnectSmashChatGPT);
                })
        });
        let disconnect_chatgpt_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Disconnect", DangerSecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(AISettingsPageAction::DisconnectSmashChatGPT);
                })
        });
        let ollama_url = AISettings::as_ref(ctx).smash_ollama_url.value().clone();
        let ollama_url_input = ctx.add_typed_action_view(move |ctx| {
            let mut input = SubmittableTextInput::new(ctx)
                .validate_on_edit(|value| normalize_smash_provider_url(value).is_ok());
            input.set_placeholder_text(DEFAULT_SMASH_OLLAMA_URL, ctx);
            input.editor().update(ctx, |editor, ctx| {
                editor.set_buffer_text(&ollama_url, ctx);
            });
            input
        });
        ctx.subscribe_to_view(&ollama_url_input, |_, _, event, ctx| {
            if let SubmittableTextInputEvent::Submit(url) = event {
                ctx.dispatch_typed_action(&AISettingsPageAction::SetSmashOllamaUrl(url.clone()));
            }
        });

        let lm_studio_url = AISettings::as_ref(ctx).smash_lm_studio_url.value().clone();
        let lm_studio_url_input = ctx.add_typed_action_view(move |ctx| {
            let mut input = SubmittableTextInput::new(ctx)
                .validate_on_edit(|value| normalize_smash_provider_url(value).is_ok());
            input.set_placeholder_text(DEFAULT_SMASH_LM_STUDIO_URL, ctx);
            input.editor().update(ctx, |editor, ctx| {
                editor.set_buffer_text(&lm_studio_url, ctx);
            });
            input
        });
        ctx.subscribe_to_view(&lm_studio_url_input, |_, _, event, ctx| {
            if let SubmittableTextInputEvent::Submit(url) = event {
                ctx.dispatch_typed_action(&AISettingsPageAction::SetSmashLmStudioUrl(url.clone()));
            }
        });

        Self {
            connect_chatgpt_button,
            disconnect_chatgpt_button,
            ollama_url_input,
            lm_studio_url_input,
        }
    }
    ...
    fn provider_row(
        ...
        let mut row = Flex::row()
            ...
            .with_child(
                Flex::column()
                    .with_child(
                        Text::new_inline(name, appearance.ui_font_family(), CONTENT_FONT_SIZE)
    ...
    fn local_provider_row(
        ...
        Container::new(
            Flex::column()
                ...
                .with_child(
                    Text::new_inline(description, appearance.ui_font_family(), CONTENT_FONT_SIZE)
                        .with_color(appearance.theme().disabled_ui_text_color().into())
                        .finish(),
                )
...
#[cfg(not(target_family = "wasm"))]
impl SettingsWidget for SmashProvidersWidget {
    type View = AISettingsPageView;

    fn search_terms(&self) -> &str {
        "smash ai providers chatgpt subscription oauth ollama lm studio local models"
    }
    ...
    fn render(
        ...
        let connected = smash_chatgpt::is_connected();
        let chatgpt_action = if connected {
            self.disconnect_chatgpt_button.as_ref(app).render(app)
        } else {
            self.connect_chatgpt_button.as_ref(app).render(app)
        };
        Flex::column()
            .with_child(
                build_sub_header(
                    appearance,
                    "Model providers",
                    Some(appearance.theme().active_ui_text_color()),
                )
            ...
            .with_child(
                Text::new(
                    "Smash connects directly to providers. It does not launch Codex, Claude, or another agent process.",
                    appearance.ui_font_family(),
                    CONTENT_FONT_SIZE,
                )
                .with_color(appearance.theme().disabled_ui_text_color().into())
            ...
            .with_child(self.provider_row(
                "ChatGPT subscription",
                "Browser OAuth · tokens stored in your system keychain",
                if connected { "Connected" } else { "Not connected" },
                Some(chatgpt_action),
                appearance,
            ))
            .with_child(self.local_provider_row(
                "Ollama",
                "Server URL used for model discovery and agent requests",
                &self.ollama_url_input,
                appearance,
            ))
            .with_child(self.local_provider_row(
                "LM Studio",
                "OpenAI-compatible server URL used for models and chat completions",
                &self.lm_studio_url_input,
                appearance,
            ))
            .finish()
    }
...
impl AgentsWidget {
    ...
    fn render_command_denylist(
        ...
        render_ai_list(
            "Command denylist",
            "Regular expressions for commands that the Smash Agent must always ask permission to execute.",
            list,
            view,
            ai_settings,
            app,
    ...
    fn render_command_allowlist(
        ...
        render_ai_list(
            "Command allowlist",
            "Regular expressions for commands that the Smash Agent may execute automatically.",
            list,
            view,
            ai_settings,
            app,
    ...
    fn render_base_model_setting(
        ...
        render_dropdown_item(
            ...
            "Base model",
            Some(
                "This is the primary model used by the Smash Agent. Smash may use another configured model when the selected model is unavailable or for auxiliary tasks such as conversation summarization.",
            ),
    ...
    fn render_codebase_context_outline_generation_setting(
        ...
        let mut codebase_context_description = vec![FormattedTextFragment::plain_text(
            "Allow the Smash Agent to generate an outline of your codebase for context. No code is stored on a server.",
        )];
        if ChannelState::channel() != Channel::Oss {
            codebase_context_description.push(FormattedTextFragment::hyperlink(
                " Learn more",
    ...
    fn render_mcp_permissions_zero_state(
        ...
        let subtext = {
            let mut subtext_fragments = vec![
                FormattedTextFragment::plain_text(
                    "You haven't added any MCP servers yet. Once you do, you can control how much autonomy the Smash Agent has when using them. ",
                ),
    ...
    fn render_mcp_permissions_with_servers(
            ...
            || current_mcp_setting == ActionPermission::AgentDecides
        {
            let allowlist = self.render_mcp_list(
                "MCP allowlist",
                "Allow the Smash Agent to call these MCP servers.",
            ...
            || current_mcp_setting == ActionPermission::AgentDecides
        {
            let denylist = self.render_mcp_list(
                "MCP denylist",
                "The Smash Agent always asks permission before calling MCP servers on this list.",
...
impl AIInputWidget {
    fn render_natural_language_detection_section(
        ...
        if FeatureFlag::AgentView.is_enabled() {
            let mut autodetection_description_fragments = vec![FormattedTextFragment::plain_text(
                "Encountered an incorrect detection?",
            )];
            if ChannelState::channel() != Channel::Oss {
                autodetection_description_fragments.push(FormattedTextFragment::hyperlink(
                    " Let us know",
            ...
            let mut natural_language_detection_description_fragments = vec![
                FormattedTextFragment::plain_text(
            ...
            if ChannelState::channel() != Channel::Oss {
                natural_language_detection_description_fragments.push(
                    FormattedTextFragment::hyperlink(
                        " Let us know",
                        "https://warpdotdev.typeform.com/to/offrTIpq",
                    ),
...
impl SettingsWidget for MCPServersWidget {
    ...
    fn render(
        ...
        let agent_name = if ChannelState::channel() == Channel::Oss {
            "Smash Agent"
        } else {
            "Warp Agent"
        };
        let mut mcp_description = vec![FormattedTextFragment::plain_text(format!(
            "Add MCP servers to extend the {agent_name}'s capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins."
        ))];
        if ChannelState::channel() != Channel::Oss {
            mcp_description.push(FormattedTextFragment::hyperlink(
                " Learn more",
                "https://docs.warp.dev/agents/capabilities/mcp",
            ));
        }
        ...
        let file_based_mcp_toggle = if FeatureFlag::FileBasedMcp.is_enabled() {
            Some(
                Flex::column()
                    ...
                    .with_child({
                        let mut file_based_mcp_description_fragments = vec![
                            FormattedTextFragment::plain_text(
                        ...
                        if ChannelState::channel() != Channel::Oss {
                            file_based_mcp_description_fragments.push(
                                FormattedTextFragment::hyperlink(
...
impl SettingsWidget for RulesWidget {
    ...
    fn render(
        ...
        let mut rules_description = vec![FormattedTextFragment::plain_text(format!(
            "Rules help the {agent_name} follow your conventions, whether for codebases or specific workflows."
        ))];
        if ChannelState::channel() != Channel::Oss {
            rules_description.push(FormattedTextFragment::hyperlink(
                " Learn more",
...
impl VoiceWidget {
    fn render_voice_section(
        ...
        let voice_input_description_text_fragments = vec![
            FormattedTextFragment::plain_text(
                "Voice input lets you control Smash by speaking directly to your terminal (powered by ",
            ),
        ...
        let label = render_body_item_label::<AISettingsPageAction>(
            ...
            Some(AdditionalInfo {
                ...
                tooltip_override_text: Some(
                    "Requires the Smash integration for your coding agent".to_owned(),
                ),
            }),
            LocalOnlyIconState::for_setting(
