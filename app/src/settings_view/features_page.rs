...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    toggle_binding_pairs.push(ToggleSettingActionPair::new(
        "reuse existing SSH ControlMaster in the Smash SSH wrapper",
        builder(SettingsAction::FeaturesPageToggle(
    ...
    if DefaultTerminal::can_warp_become_default() {
        app.register_fixed_bindings([FixedBinding::empty(
            if ChannelState::channel() == Channel::Oss {
                "Make Smash the default terminal"
            } else {
...
impl SettingsWidget for SessionRestorationWidget {
    ...
    fn render(
        ...
        let labeled_switch = render_body_item::<FeaturesPageAction>(
            ...
            (ChannelState::channel() != Channel::Oss).then(|| AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
        ...
        if app.is_wayland() {
            ...
            let mut wayland_notice = Flex::row().with_child(message);
            if ChannelState::channel() != Channel::Oss {
                wayland_notice.add_child(
                    ui_builder
                        .link(
                            "See docs.".to_owned(),
                            Some(
...
impl SettingsWidget for LoginItemWidget {
    ...
    fn render(
        ...
        let ui_builder = appearance.ui_builder();
        #[cfg(target_os = "macos")]
        let label = if ChannelState::channel() == Channel::Oss {
            "Start Smash at login (requires macOS 13+)"
        } else {
            "Start Warp at login (requires macOS 13+)"
        };
        #[cfg(not(target_os = "macos"))]
        let label = if ChannelState::channel() == Channel::Oss {
            "Start Smash at login"
        } else {
            "Start Warp at login"
        };
        render_body_item::<FeaturesPageAction>(
    ...
    fn render(
        ...
        if default_terminal.is_warp_default() {
            ...
            ui_builder
                .link(
                    if ChannelState::channel() == Channel::Oss {
                        "Make Smash the default terminal".to_string()
                    } else {
...
impl SettingsWidget for DesktopNotificationsWidget {
    ...
    fn render(
        ...
        column.add_child(render_body_item::<FeaturesPageAction>(
            if ChannelState::channel() == Channel::Oss {
                "Receive desktop notifications from Smash".into()
            } else {
                ...
                Flex::row()
                    .with_children([
                        ui_builder
                            ...
                            .finish(),
                        if ChannelState::channel() == Channel::Oss {
                            ui_builder.span(String::new()).build().finish()
                        } else {
                            ui_builder
                                .link(
                                    "See docs.".to_owned(),
...
impl SettingsWidget for WindowSystemWidget {
    ...
    fn render(
        ...
        if view.force_x11_changed {
            secondary_text.push_str(if ChannelState::channel() == Channel::Oss {
                "\n\nRestart Smash for changes to take effect."
            } else {
