...
const DESCRIPTION_TEXT: &str = "Add MCP servers to extend the Smash Agent. MCP servers expose data sources and tools through a standard interface. Add a custom server or start with a preset.";
...
impl MCPServersListPageView {
    ...
    fn render_file_based_mcp_section(
        ...
        let mut file_based_mcp_description_fragments = vec![FormattedTextFragment::plain_text(
        ...
        if ChannelState::channel() != Channel::Oss {
            file_based_mcp_description_fragments.push(FormattedTextFragment::hyperlink(
    ...
    fn render_page_body(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let mut description_fragments = vec![FormattedTextFragment::plain_text(DESCRIPTION_TEXT)];
        if ChannelState::channel() != Channel::Oss {
            description_fragments.push(FormattedTextFragment::hyperlink(
                "Learn more.",
                ...
                && filtered_file_based_cards.is_empty()
            {
                ...
                if !shared_server_cards.is_empty() {
                    ...
                    let shared_by_text = match team_name {
                        Some(name) => format!("Shared by Smash and {name}"),
                        None => "Shared by Smash from another device".to_string(),
                    };
