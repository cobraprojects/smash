...
impl WarpifyPageView {
    ...
    fn build_page(ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        {
            categories.push(
                Category::new("SSH", vec![Box::new(SSHWidget::default())]).with_subtitle(
                    if ChannelState::channel() == Channel::Oss {
                        "Smashify your interactive SSH sessions."
                    } else {
...
impl TitleWidget {
    fn render_top_of_page(&self, appearance: &Appearance, _app: &AppContext) -> Box<dyn Element> {
        let is_smash = ChannelState::channel() == Channel::Oss;
        let mut warpify_description = vec![FormattedTextFragment::plain_text(if is_smash {
            "Configure whether Smash attempts to “Smashify” certain shells by adding support for blocks and input modes."
        } else {
            "Configure whether Warp attempts to “Warpify” (add support for blocks, input modes, etc) certain shells. "
        })];
        if !is_smash {
            warpify_description.push(FormattedTextFragment::hyperlink(
                "Learn more",
        ...
        Flex::column()
            .with_child(render_page_title(
                if is_smash { "Smashify" } else { "Warpify" },
                HEADER_FONT_SIZE,
                appearance,
            ))
    ...
    fn render(
            ...
            add_setting(
                ...
                move || {
                    Container::new(render_dropdown_item(
                        ...
                        Some(if is_smash {
                            "Controls how Smash installs its SSH extension when a remote host does not have it."
                        } else {
        ...
        add_setting(
            ...
            move || {
                ...
                column.add_child(
                    ui_builder
                        .paragraph(if is_smash {
                            "Attach to an existing SSH ControlMaster instead of creating a Smash-owned one. Takes effect in new tabs.".to_owned()
                        } else {
                            "Attach to a live SSH ControlMaster you already have configured for the destination host instead of creating a Warp-owned one. Takes effect in new tabs.".to_owned()
                        })
                        .with_style(UiComponentStyles {
