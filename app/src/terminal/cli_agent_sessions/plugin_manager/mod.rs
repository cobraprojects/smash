...
/// Manages the Smash notification plugin for a specific CLI agent.
///
...
pub(crate) trait CliAgentPluginManager: Send + Sync {
    /// The minimum plugin version required by this Smash build.
    ...
    /// Whether the Smash notification plugin is installed.
    ...
    /// Install the Smash notification plugin.
    ...
    /// Update the Smash notification plugin to the latest version.
    /// Default returns an error — only agents with `can_auto_install() == true` should override.
    ...
    fn install_success_message(&self) -> &'static str {
        "Smash plugin installed. Please restart the session to activate."
    }

    /// Toast message shown after a successful auto-update.
    fn update_success_message(&self) -> &'static str {
        "Smash plugin updated. Please restart the session to activate."
    }
...
/// doesn't have Smash notification plugin support.
