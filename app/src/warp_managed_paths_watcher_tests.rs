...
use super::{
    filter_repository_update_by_prefix, smash_home_mcp_config_file_path, smash_home_skills_dir,
    warp_managed_mcp_config_path, warp_managed_skill_dirs,
};

#[test]
fn warp_managed_skill_dirs_contains_only_smash_home_path() {
    let dirs = warp_managed_skill_dirs();
    match smash_home_skills_dir() {
        Some(smash_home_skills_dir) => assert_eq!(dirs, vec![smash_home_skills_dir]),
        None => assert!(dirs.is_empty()),
    }
}

#[test]
fn warp_managed_mcp_config_path_contains_only_smash_home_path() {
    match (
        home_dir(),
        smash_home_mcp_config_file_path(),
        warp_managed_mcp_config_path(),
    ) {
        (Some(home_dir), Some(smash_home_mcp_config_path), Some(path)) => {
            assert_eq!(path.root_path, home_dir);
            assert_eq!(path.config_path, smash_home_mcp_config_path);
        }
        (_, _, None) => {}
        _ => panic!("Expected Smash MCP path when home directory is available"),
    }
}

#[test]
fn filter_repository_update_by_prefix_keeps_only_matching_paths() {
    let skills_dir = PathBuf::from("/tmp/.smash-local/skills");
    let other_dir = PathBuf::from("/tmp/.smash-local/worktrees/repo");
...
fn filter_repository_update_by_prefix_converts_cross_boundary_moves() {
    let skills_dir = PathBuf::from("/tmp/.smash-local/skills");
    let skill_file = skills_dir.join("deploy").join("SKILL.md");
    let ignored_file = PathBuf::from("/tmp/.smash-local/worktrees/repo/SKILL.md");
