...
```sh
for i in {0..100}; do
    SMASHUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 RUST_BACKTRACE=full SMASH_SHELL_PATH=/bin/bash cargo run -p integration -- test_simple_example
...
```
  SMASHUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS="1" cargo run --bin integration -- test_simple_example
```

The `SMASHUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS="1"` will force the new terminal window to open, which helps a lot when iterating on your integration test implementation!

### Known issues / limitations
* To determine (from the `TestStep`) which shell is used for the test, you can try checking the `SMASH_SHELL_PATH` environment variable (that works within CI) or check the passwd entry for the user (for local runs).
* Similarly, you can run the test with a specific shell by setting `SMASH_SHELL_PATH` and then running the test. If you're running with fish, you also need to pass `--features fish_shell` until that feature flag is removed. For example: `SMASH_SHELL_PATH=/usr/local/bin/fish`, then `cargo run --bin integration --features fish_shell -- test_simple_example`.
