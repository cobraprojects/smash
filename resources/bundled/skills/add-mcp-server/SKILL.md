---
name: agent-add-mcp
description: Use this skill when helping users add MCP servers to their Smash configuration.
---

# Adding MCP Servers to Smash

Smash supports MCP servers via native config files. Follow these steps when helping a user add an MCP server.
...
Config file paths:
- **Global (user-scoped):** `~/.smash/.mcp.json`
- **Project-scoped:** `{repo_root}/.smash/.mcp.json`
...
By default, Smash spawns stdio servers from the directory the config was discovered in:
- Project-scoped configs (`{repo_root}/.smash/.mcp.json`) run from the repo root.
- Global configs (`~/.smash/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
...
For environment variables containing secrets, use `${VAR_NAME}` syntax — Smash will substitute the value from the user's environment at runtime.

## Notes

- Smash auto-detects changes to `.mcp.json` files on save — no restart required.
- Configured servers appear in Smash Settings under MCP, labeled **"Detected from Smash"**.
