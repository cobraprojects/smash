...
Tab configs live under `~/.smash/tab_configs/` for standard Smash builds. Non-stable builds use a channel-specific variant such as `~/.smash-<channel>/tab_configs/`. Do NOT use `~/Library/Application Support/` — that is the wrong location.

To find the active data directory, inspect which `~/.smash*` directories exist on the user's machine. Each `.toml` file defines a single tab layout that appears in the `+` menu. When the user selects one, Smash opens a new tab with the configured pane layout and runs any specified commands.
...
### With parameters
Smash-generated worktree configs default to `~/.smash/worktrees/<repo-name>/<worktree-name>`. When writing a reusable config by hand, derive the repo-name segment at shell runtime with `$(basename {{repo}})`.
...
directory = "{{repo}}"
commands = [
  "git worktree add -b {{branch_name}} $HOME/.smash/worktrees/$(basename {{repo}})/{{branch_name}} {{base_branch}}",
  "cd $HOME/.smash/worktrees/$(basename {{repo}})/{{branch_name}}",
]
